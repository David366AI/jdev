...
public abstract class CodeAidPopup
{
     ...
     */
    public void show(JEditTextArea textArea) {
        show(textArea, -1, -1);
    }
    ...
    public void show(JEditTextArea aTextArea, int x, int y) {
        ...
        if (currentPopup != null) {
            if (codeaid.CodeAidPlugin.isDebugEnabled() ) Log.log(Log.DEBUG, this,
                "Not displaying popup because another popup or this popup is already displayed");
            return;
        }

        textArea = aTextArea;
        PopupWindow window = getPopupWindow(getView());
        calculateBounds(textArea, window, x, y);
    ...
    public void dispose() {
        ...
        currentPopup = null;
        typedText = "";
        if (codeaid.CodeAidPlugin.isDebugEnabled() ) Log.log(Log.DEBUG, this, "Disposing popup");
